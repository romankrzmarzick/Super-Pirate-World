...
def import_image(*path, alpha=True, format='png'):
    ...
    surf = pg.image.load(full_path).convert_alpha() if alpha else pg.image.load(full_path).convert()
    return surf
...
def import_tile_map(cols, rows, *path):
    ...
    for col in range(cols):
        for row in range(rows):
            cutout_rect = pg.Rect(col * cell_width, row * cell_height, cell_width, cell_height)
